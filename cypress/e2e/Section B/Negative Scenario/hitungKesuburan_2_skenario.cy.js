...
Cypress.on('uncaught:exception', (err, runnable) => {
  ...
  describe('Login', () => {
    ...
    it('user want to "hitung kesuburan" with the number "lama haid" which is not allowed', () => {
    ...
    it('user want to "hitung kesuburan" with the number "siklus haid" which is not allowed', () => {
      cy.url().should('eq', 'https://www.ruangmom.com/kids-and-parenting');
      cy.get('a.nav-link[href="/pernikahan"]').click();
      cy.get('button.ant-btn:contains("Coba hitung")').click({ force: true });
      cy.get('input.ant-calendar-picker-input[placeholder="Pilih tanggal"]').invoke('val', pastDate.toISOString().split('T')[0]);
      cy.get('input.ant-input[placeholder="Isi dengan Angka 1 hingga 10"]').click({ force: true }).type(faker.random.number({ min: 2, max: 10 }), { delay: 0 });
      cy.get('div.ant-col.ant-form-item-control-wrapper input#period_calculator_siklusHaid')
      .should('exist')
      .click({ force: true })
      .type(faker.random.number({ min: 41 }).toString(), { delay: 0 });                                                                                                    
      cy.get('button.ant-btn:contains("Hitung")').click({ force: true });
      cy.contains('div.ant-form-explain', 'Isi dengan Angka, 22 hingga 40!')
      .should('be.visible');    
  });
  })
